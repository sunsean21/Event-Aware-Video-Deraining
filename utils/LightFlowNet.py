...
################################################################################
#                                                                              #
# The code is from https://github.com/lhao0301/pytorch-liteflownet3            #
# lhao003 owns the copyright                                                   #
#                                                                              #
################################################################################
################################################################################
################################################################################
...
class LightFlowNet(torch.nn.Module):
    def __init__(self):
        ...
        self.load_state_dict(torch.load('utils/network-{}.pytorch'.format(arguments_strModel)))
    # end
...
if __name__ == '__main__':
    arguments_strFirst = './images/00001.jpg'
    arguments_strSecond = './images/00002.jpg'
    ...
    # warp img2 to img1 by the optical flow
    warped = backwarp(tenSecond.unsqueeze(0).cuda(), tenOutput.unsqueeze(0).cuda())
    warped = warped.squeeze(0).permute(1,2,0).cpu().numpy()[:,:,::-1] * 255. 
    print(warped.shape, type(warped))
    img = PIL.Image.fromarray(warped.astype(numpy.uint8))
    img.save("warped.png")
    # save optical flow
